...
if 'title' not in [k for k in stemmed_documents.index_information().keys()]:
    stemmed_documents.create_index([("title", ASCENDING)])
...
def get_filename(title):
    doc = stemmed_documents.find_one({"title" : title})
    return doc["filename"]

def get_doc_display(doc):
    filename = get_filename(doc.title) + ".txt"
    if filename[0].isdigit(): # got an EEBO book
        base_dir = '/home/skipper/school/seaboldgit/dittmar/books/data/eebobooks/'
    elif filename[0].lower() == 'k': # got an ECCO book
        base_dir = '/home/skipper/school/seaboldgit/dittmar/books/data/eccobooks/plainText'
    content = open(os.path.join(base_dir, filename), 'r').read()
    content = "Filename on disk: " + filename + "\n\n"
    #TODO: spruce this up a lot for html display
    return content
